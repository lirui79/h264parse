...
/**
 计算CeilLog2(inputVal)
 */
static unsigned int calculateCeilLog2(unsigned int inputVal) {
    unsigned int tmpVal = inputVal - 1;
    unsigned int ret = 0;
...
void parse_rest_elememt_of_sliceHeader(bs_t *b, sps_t *sps, pps_t *pps, slice_t *slice) {
    ...
    if (sps->separate_colour_plane_flag == 1) {
        ...
        slice_header->colour_plane_id = 0;// PLANE_Y
    }
...
void parse_slice(bs_t *b, sps_t *sps, pps_t *pps, slice_t *slice) {
    // 0.先解析slice_header
    parse_slice_header(b, sps, pps, slice);
}